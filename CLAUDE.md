...
- `translateImport.mts` - German translation for URL-imported recipes via Gemini (`POST /api/recipes/import` responds with `translated`): non-German results are translated best-effort — skipped without `GEMINI_API_KEY` and for already-German recipes, falls back to the original on Gemini errors; model overridable via `RECIPE_TRANSLATE_MODEL`
