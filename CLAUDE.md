...
- `recipeHtmlFallback.mjs` - fallback for pages without JSON-LD: schema.org microdata (`itemprop`) first, then heading-based heuristics ("Zutaten"/"Zubereitung" + lists); the import response carries `parser: 'jsonld' | 'html'` so the form can show a review hint
- `photoImport.mjs` - import recipes from a photo (cookbook page, handwritten note) via a vision model with structured output (`POST /api/recipes/import/photo`): OpenAI first (`OPENAI_API_KEY`), Anthropic as fallback on technical failures (`ANTHROPIC_API_KEY`); 503 without any key. Models overridable via `PHOTO_IMPORT_OPENAI_MODEL`/`PHOTO_IMPORT_MODEL`, tests mock both APIs via `OPENAI_BASE_URL`/`ANTHROPIC_BASE_URL`
